...
; ── 自动大循环配置 ──
AUTO_MASTERY_LOOPS := 102   ; 每周期刷熟练度圈数
AUTO_BUYCAR_TIMES  := 30    ; 每周期买车次数
...
Log(msg) {
    global logBox
    line := "[" FormatTime(, "HH:mm:ss") "] " msg "`r`n"
    ; 将光标移动到文本末尾 (EM_SETSEL)
    SendMessage(0x00B1, -1, -1, logBox)
    ; 在光标处插入新文本，不影响现有文本 (EM_REPLACESEL)
    SendMessage(0x00C2, False, StrPtr(line), logBox)
    ; 滚动到最底部 (WM_VSCROLL)
...
GetWindowPixelColor(hwnd, x, y) {
    ...
    ; 【修复点 1】：保存旧的位图对象
    oldBmp := DllCall("SelectObject", "Ptr", hMDC, "Ptr", hBmp, "Ptr") 
    
    DllCall("PrintWindow", "Ptr", hwnd, "Ptr", hMDC, "UInt", 2)
    col := DllCall("GetPixel", "Ptr", hMDC, "Int", x, "Int", y, "UInt")
    
    ; 【修复点 2】：将旧的位图对象塞回 DC，这样后面的 DeleteObject 才能真正生效
    DllCall("SelectObject", "Ptr", hMDC, "Ptr", oldBmp, "Ptr") 
